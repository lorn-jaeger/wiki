...
def relative_link(from_path: Path, to_path: Path) -> str:
    """Return a POSIX-style relative link between two repository-rooted paths."""
    return Path(
        os.path.relpath(
            to_path,
...
def build_navigation(
    current_md: Path, ordered_pages: list[Path], index_html: Path
) -> str:
    """Return a markdown navigation row for the given page."""
    try:
        idx = ordered_pages.index(current_md)
    except ValueError:
        return ""

    prev_md = ordered_pages[idx - 1] if idx > 0 else None
    next_md = ordered_pages[idx + 1] if idx + 1 < len(ordered_pages) else None

    current_html = current_md.with_suffix(".html")
    prev_link = (
        f"[Prev]({relative_link(current_html, prev_md.with_suffix('.html'))})"
        if prev_md
        else "Prev"
    )
    next_link = (
        f"[Next]({relative_link(current_html, next_md.with_suffix('.html'))})"
        if next_md
        else "Next"
    )
    index_link = f"[Index]({relative_link(current_html, index_html)})"

    return f"{prev_link} · {index_link} · {next_link}"
...
def build_site() -> None:
    ...
    md_rel_paths = [md_file.relative_to(REPO_ROOT) for md_file in md_files]
    for md_file, md_rel_path in zip(md_files, md_rel_paths):
        ...
        navigation = build_navigation(md_rel_path, md_rel_paths, Path("index.html"))
        if navigation:
            markdown_text = f"{navigation}\n\n{markdown_text}\n\n{navigation}\n"
